...
RSpec.shared_examples 'routes' do |route:, actions: []|
    ...
    if [:show, :update, :destroy].include?(action)
      describe "##{action} action" do
        if actions.include?(action)
          it_behaves_like 'is routable', "#{route}/#{resource_id}", action_to_method[action], action, { id: resource_id.to_s, format: :json }
        else
      ...
      describe "##{action} action" do
        if actions.include?(action)
          it_behaves_like 'is routable', "#{route}", action_to_method[action], action, format: :json
        else
